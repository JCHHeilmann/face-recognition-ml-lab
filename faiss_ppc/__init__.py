...
def handle_Index(the_class):
    ...
    def replacement_search_and_reconstruct(self, x, k, D=None, I=None, R=None):
        ...
        self.search_and_reconstruct_c(
            n, swig_ptr(x), k, swig_ptr(D), swig_ptr(I), swig_ptr(R)
        )
        return D, I, R
...
def pairwise_distances(xq, xb, mt=METRIC_L2, metric_arg=0):
    ...
    if mt == METRIC_L2:
        ...
        pairwise_extra_distances(
            d, nq, swig_ptr(xq), nb, swig_ptr(xb), mt, metric_arg, swig_ptr(dis)
        )
    return dis
...
def knn(xq, xb, k, distance_type=METRIC_L2):
    ...
    if distance_type == METRIC_L2:
        ...
        knn_L2sqr(swig_ptr(xq), swig_ptr(xb), d, nq, nb, heaps)
        ...
        heaps.val = swig_ptr(D)
        heaps.ids = swig_ptr(I)
        knn_inner_product(swig_ptr(xq), swig_ptr(xb), d, nq, nb, heaps)
    return D, I
...
class ResultHeap:
    ...
    def add_result(self, D, I):
        ...
        self.heaps.addn_with_ids(self.k, swig_ptr(D), swig_ptr(I), self.k)
